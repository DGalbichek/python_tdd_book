...
class ListViewTest(TestCase):
    ...
    def test_displays_only_items_for_that_list(self):
        ...
        other_list = List.objects.create()
        Item.objects.create(text='othery itemey 1', list=other_list)
        Item.objects.create(text='othery itemey 2', list=other_list)

        response = self.client.get(f'/lists/{correct_list.id}/')

        self.assertContains(response, 'itemey 1')
        self.assertContains(response, 'itemey 2')
        self.assertNotContains(response, 'othery itemey 1')
        self.assertNotContains(response, 'othery itemey 2')
...
class MyListsTest(TestCase):
    def test_my_lists_url_renders_my_lists_template(self):
        ...
        response = self.client.post('/lists/users/a@b.com/')
...
class ShareListTest(TestCase):
    ...
    def test_post_assigns_user_to_list(self):
        user = User.objects.create(email='a@b.com')
        list_ = List.objects.create()
        response = self.client.post(f'/lists/{list_.id}/share', data={'sharee': 'a@b.com'})
        self.assertIn('a@b.com', [x.email for x in list_.shared_with.all()])
